...
static JSValue js_native_get_host_control_base_url(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	(void)this_val; (void)argc; (void)argv;
	char url[VD_HOST_CONTROL_URL_MAX];
	settings_get_host_control_url(url, sizeof(url));
...
void register_js_lib(JSContext *ctx) {
	...
	register_js_host_control(ctx);
	register_js_instance_tree(ctx);
	...
	js_set_global_function(ctx, "nativeGetHostControlBaseUrl", js_native_get_host_control_base_url, 0);
}
