...
The Linux smoke path is single-source: local runs and GitHub Actions both use `scripts/smoke-docker.sh`, which builds the `smoke` stage from `Dockerfile`.
...
- Use `VITADECK_SMOKE_NO_BUILD=1` only when the `vitadeck-smoke` image is already current.
